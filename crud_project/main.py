...
# a = {'title': 'Платье', 'id': 36262, 'price': 2000, 'description': 'красивое платье', 
# 'created_at': '23.08.22'}
# print(a)
# a['title'] = 'велосипед'
# del a
...
def get_all_data():
    with shelve.open(FILENAME) as db:
        for key, value in db.items():
            print('--------------------------')
            print('id:' , key, '|', 'title:', value['title'], '|', 
            'price:', value['price'])
            print('--------------------------')            

def get_data_by_id():
    id_ = input('Введте id товара: ')
    with shelve.open(FILENAME) as db:
        try:
            prod = db[id_]
            print(
...
def update_data():
    ...
    with shelve.open(FILENAME, writeback=True) as db:
        try:
            ...
            prod['price'] = int(input('Введите цену: ')) or prod['price']
            prod['description'] = input('Введите описание: ') or prod['description']
        except KeyError:
            print(f'{id_} не сущетсвует')
